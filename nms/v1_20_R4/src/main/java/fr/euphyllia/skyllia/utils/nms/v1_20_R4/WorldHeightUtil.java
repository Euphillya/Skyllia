...
public final class WorldHeightUtil {
    ...
    private static final MappedRegistry<DimensionType> REGISTRY =
            (MappedRegistry<DimensionType>) MinecraftServer.getServer()
                    .registryAccess().registry(Registries.DIMENSION_TYPE).orElseThrow();

    private WorldHeightUtil() {
    }

    /**
     * Registers a custom DimensionType for the given world name + environment,
     * based on the height settings in {@code config}.
     * Must be called BEFORE the world is created via WorldCreator.
     */
    public static Holder<DimensionType> registerCustomDimension(String worldName, WorldConfig config) {
        ...
        ResourceKey<DimensionType> newKey = ResourceKey.create(
                ...
                new ResourceLocation("skyllia", worldName.toLowerCase().replace(" ", "_"))
        );
    ...
    private static Holder<DimensionType> getBaseHolder(World.Environment env) {
        return switch (env) {
            case NETHER -> REGISTRY.getHolderOrThrow(BuiltinDimensionTypes.NETHER);
            case THE_END -> REGISTRY.getHolderOrThrow(BuiltinDimensionTypes.END);
            default -> REGISTRY.getHolderOrThrow(BuiltinDimensionTypes.OVERWORLD);
        };
    }
