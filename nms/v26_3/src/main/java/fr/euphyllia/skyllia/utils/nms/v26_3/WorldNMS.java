...
public class WorldNMS extends fr.euphyllia.skyllia.api.utils.nms.WorldNMS {
    ...
    public WorldFeedback.FeedbackWorld createWorld(final WorldCreator creator, final WorldConfig worldConfig) {
        ...
        final net.minecraft.core.Holder<net.minecraft.world.level.dimension.DimensionType> holder =
                WorldHeightUtil.registerCustomDimension(creator.name(), worldConfig);
        return createWorldInternal(creator, worldConfig, holder);
    }

    private WorldFeedback.FeedbackWorld createWorldInternal(final WorldCreator creator, final WorldConfig worldConfig, final net.minecraft.core.Holder<net.minecraft.world.level.dimension.DimensionType> customHeightHolder) {
        ...
        if (worldGenSettings == null) {
            ...
            final DedicatedServerProperties.WorldDimensionData properties = new DedicatedServerProperties.WorldDimensionData(GsonHelper.parse((creator.generatorSettings().isEmpty()) ? "{}" : creator.generatorSettings()), creator.type().name().toLowerCase(Locale.ROOT));
            final WorldDimensions worldDimensions = properties.create(registryAccess);
        ...
        final WorldInfo worldInfo = new CraftWorldInfo(loadedWorldData.bukkitName(), CraftNamespacedKey.fromMinecraft(dimensionKey.identifier()), genSettingsFinal.options().seed(), primaryLevelData.enabledFeatures(), creator.environment(), customStem.type().value(), customStem.generator(), getServer().registryAccess(), loadedWorldData.uuid());
    ...
    public void resetChunk(final World craftWorld, final ChunkCoordinate position) {
        ...
        for (final Entity entity : FeatureHooks.getChunkEntities(nms, chunkX, chunkZ)) {
    ...
    public List<Entity> getEntities(final World craftWorld, final @Nullable Entity except, final BoundingBox boundingBox, final Predicate<? super Entity> filter) {
        ...
        final List<net.minecraft.world.entity.Entity> entityList = new ArrayList<>();
