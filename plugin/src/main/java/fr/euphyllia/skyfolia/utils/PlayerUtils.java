...
public class PlayerUtils {
    ...
    public static void setOwnWorldBorder(Main main, Player player, @NotNull Location centerBorder, @NotNull String colorBorder, double borderSize, int warningBlocks, int warningTime) throws UnsupportedMinecraftVersionException {
        ...
        switch (versionMC) {
            case "v1_19_R3" ->
                    fr.euphyllia.skyfolia.utils.nms.v1_19_R3.PlayerNMS.setOwnWorldBorder(main, player, centerBorder, colorBorder, borderSize, warningBlocks, warningTime);
            case "v1_20_R1" ->
                    fr.euphyllia.skyfolia.utils.nms.v1_20_R1.PlayerNMS.setOwnWorldBorder(main, player, centerBorder, colorBorder, borderSize, warningBlocks, warningTime);
            case "v1_20_R2" ->
                    fr.euphyllia.skyfolia.utils.nms.v1_20_R2.PlayerNMS.setOwnWorldBorder(main, player, centerBorder, colorBorder, borderSize, warningBlocks, warningTime);
            default ->
                    throw new UnsupportedMinecraftVersionException("Version %s not supported !".formatted(versionMC));
        }
    }
