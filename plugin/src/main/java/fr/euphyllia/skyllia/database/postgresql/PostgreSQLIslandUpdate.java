...
public class PostgreSQLIslandUpdate extends IslandUpdateQuery {
    ...
    public PostgreSQLIslandUpdate(DatabaseLoader databaseLoader) {
        this.databaseLoader = databaseLoader;
    }

    @Override
    public Boolean updateDisable(Island island, boolean disable) {
        int affected = SQLExecute.update(databaseLoader, UPDATE_DISABLE_ISLAND, List.of(disable, island.getId()));
        return affected != 0;
    }

    @Override
    public Boolean updatePrivate(Island island, boolean privateIsland) {
        int affected = SQLExecute.update(databaseLoader, UPDATE_PRIVATE_ISLAND, List.of(privateIsland, island.getId()));
        return affected != 0;
    }

    @Override
    public Boolean isDisabledIsland(Island island) {
        Boolean disabled = SQLExecute.queryMap(databaseLoader, SELECT_STATUS_ISLAND, List.of(island.getId()), rs -> {
            try {
    ...
    public Boolean isPrivateIsland(Island island) {
        Boolean priv = SQLExecute.queryMap(databaseLoader, SELECT_PRIVATE_ISLAND, List.of(island.getId()), rs -> {
            try {
    ...
    public Boolean setMaxMemberInIsland(Island island, int newValue) {
        int affected = SQLExecute.update(databaseLoader, UPDATE_MEMBERS_ISLAND, List.of(newValue, island.getId()));
        return affected != 0;
    }

    @Override
    public Boolean setSizeIsland(Island island, double newValue) {
        int affected = SQLExecute.update(databaseLoader, UPDATE_SIZE_ISLAND, List.of(newValue, island.getId()));
        return affected != 0;
    }

    @Override
    public Boolean setLockedIsland(Island island, boolean locked) {
        int affected = SQLExecute.update(databaseLoader, UPDATE_LOCKED_ISLAND, List.of(locked, island.getId()));
        return affected != 0;
    }

    @Override
    public Boolean isLockedIsland(Island island) {
        Boolean locked = SQLExecute.queryMap(databaseLoader, SELECT_LOCKED_ISLAND, List.of(island.getId()), rs -> {
            try {
